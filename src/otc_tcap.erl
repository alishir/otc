...
decode_message({'abort', #{dtid := DT} = TC}) ->
    Reason =
        case maps:get(reason, TC, undefined) of
            undefined -> undefined;
            #{'p-abortCause' := P} -> P;
            #{'u-abortCause' := U} -> U
        end,
...
decode_dialogue(#{
    dialoguePortion := #{'direct-reference' := DR, encoding := {'single-ASN1-type', Enc}}
}) ->
    ...
    case DiaType of
        ...
        dialogueAbort ->
            Base#{
                abort_source => maps:get('abort-source', Dia0),
                user_information => maps:get('user-information', Dia0, undefined)
            };
        unidialoguePDU ->
            {AC, ACN} = parse_application_context_name(Dia0),
...
encode_dialogue(#{type := dialogueRequest} = Map) ->
    ...
    Dia = #{
        'protocol-version' => maps:get(supported_versions, Map, [version1]),
...
encode_dialogue(#{type := dialogueAbort} = Map) ->
    Dia = #{
        'abort-source' => maps:get(abort_source, Map, undefined),
        'user-information' => maps:get(user_information, Map, undefined)
    },
    Dialogue = remove_undefined_field('user-information', Dia),
...
decode_component({returnResultNotLast, C}) ->
    InvokeId = parse_invoke_id(maps:get(invokeId, C, absent)),
    Base = C#{
        component_type => returnResultNotLast,
        invokeId => InvokeId
    },
    case C of
        #{result := Result} ->
...
remove_undefined_field(Field, Map) when
    undefined =:= erlang:map_get(Field, Map);
